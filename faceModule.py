...
class faceDetect(object):
    ...
    def showResult(self):
        ...
        draw.line([( self.faceLandmark['left_eye_center']['x'],self.faceLandmark['left_eye_center']['y'])
                      ,(self.faceLandmark['right_eye_center']['x'],self.faceLandmark['left_eye_center']['y'])],
                  fill='green',width=2)
        draw.line([( (self.faceLandmark['left_eyebrow_right_corner']['x']+self.faceLandmark['right_eyebrow_left_corner']['x'])/2,self.faceLandmark['left_eye_center']['y'])
...
def detect(fileUrl,key,apiKey):
    ...
    if d.status:
        ...
        comment = d.message()
        status = d.status
        result ={'status':status,'grades':grades,'imgResult':imgResult,'comment':comment}
        return result
    else:
        status = d.status
        error = d.error
